...
public:
    ...
    public:
        ...
        {
            CVMessageHeader(uint8_t sof, uint8_t commandID, uint8_t dataLength)
                : startOfFrame(sof),
                  commandID(commandID),
        ...
        {
            GameStatusMessage()
                : CVMessageHeader(START_OF_FRAME, GAME_STATUS_MESSAGE, DATA_LEN_GAME){};
            uint16_t robotType;
            uint16_t redStdHP;
            uint16_t redHroHP;
            uint16_t redStyHP;
        ...
        {
            TurretMessage()
                : CVMessageHeader(START_OF_FRAME, TURRET_FEEDBACK_MESSAGE, DATA_LEN_TURRET){};
            int16_t pitch;
            int16_t yaw;
        } modm_packed;
        ...
        {
            PositionMessage()
                : CVMessageHeader(START_OF_FRAME, POSITION_FEEDBACK_MESSAGE, DATA_LEN_POSITION){};
            int16_t Ax;
            int16_t Ay;
            int16_t Az;
            int16_t Gx;
    ...
    public:
         ...
         * Angles given in millirad in the local body frame (Origin is barrel straight ahead,
         * parallel to ground)
         */
        struct TurretData
        {
