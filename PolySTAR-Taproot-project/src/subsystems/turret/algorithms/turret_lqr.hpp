...
public:
    ...
    float updateYaw(float yawAngle, float yawRate, float yawRef) const;
    float updatePitch(float pitchAngle, float pitchRate, float pitchRef) const;

    // Gravity term alone, applied by the subsystem so the sign convention
    // lives in one place (the subsystem).
    float gravityFeedforward(float pitchAngle) const;
