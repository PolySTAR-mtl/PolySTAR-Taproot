...
void ChassisSubsystem::checkPowerConsumption()
{
...
void ChassisSubsystem::updateRpmPid(
...
void ChassisSubsystem::updateRpmSetpoint()
{
    ...
    if (xInputRamp.isTargetReached() == false)
    {
        xInputRamp.update(RAMP_SLOPE * dt);
    }
    if (yInputRamp.isTargetReached() == false)
    {
        yInputRamp.update(RAMP_SLOPE * dt);
    }
    if (rInputRamp.isTargetReached() == false)
    {
        rInputRamp.update(RAMP_SLOPE * dt);
    }
...
void ChassisSubsystem::setTargetOutput(float x, float y, float r)
{
    if (powerReached)
    {
        x *= powerReductionFactor;
        y *= powerReductionFactor;
        r *= powerReductionFactor;
    }
    
    // TODO : review this, should we update the ramp target or the direct value returned?
...
void ChassisSubsystem::sendCVUpdate()
{
    ...
    int16_t frontLeftRevolutions = (frontLeftMotor.getEncoderUnwrapped() - frontLeftEncoder) /
                                   tap::motor::DjiMotor::ENC_RESOLUTION;
    uint16_t frontRightEncoder = frontRightMotor.getEncoderWrapped();
    int16_t frontRightRevolutions = (frontRightMotor.getEncoderUnwrapped() - frontRightEncoder) /
                                    tap::motor::DjiMotor::ENC_RESOLUTION;
    uint16_t backLeftEncoder = backLeftMotor.getEncoderWrapped();
    int16_t backLeftRevolutions = (backLeftMotor.getEncoderUnwrapped() - backLeftEncoder) /
                                  tap::motor::DjiMotor::ENC_RESOLUTION;
    uint16_t backRightEncoder = backRightMotor.getEncoderWrapped();
    int16_t backRightRevolutions = (backRightMotor.getEncoderUnwrapped() - backRightEncoder) /
                                   tap::motor::DjiMotor::ENC_RESOLUTION;
